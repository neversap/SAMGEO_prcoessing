    ...
    if architecture in {"unet", "unet_effb3", "smp_unet"}:
        try:
            ...
            raise ModuleNotFoundError(
                "inhouse inference requires segmentation-models-pytorch for U-Net inference"
            ) from exc
        return smp.Unet(
...
def load_checkpoint(model: nn.Module, checkpoint_path: str, device: torch.device, strict: bool = False) -> dict[str, int]:
