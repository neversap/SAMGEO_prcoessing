...
def quadify_component_rotated(
    ...
    local_mask, center, inverse_rotation = component_to_local_mask(component, angle)
    rectangles = decompose_local_rectangles(local_mask)
    if not rectangles:
        return rasterize_polygon(base_polygon, component.shape)

    output = np.zeros_like(component, dtype=np.uint8)
    for rectangle in rectangles:
        polygon = local_bounds_to_polygon(rectangle, center, inverse_rotation)
        fill_polygon(output, polygon)
    output = cover_uncovered_regions(output, component, angle)
    output = close_output_mask(output)
    return output.astype(bool)


def component_to_local_mask(
    component: np.ndarray,
    angle_degrees: float,
    ...
    local_points, inverse_rotation = to_local_points(points, center, angle_degrees)
    min_x, min_y = np.floor(local_points.min(axis=0)).astype(int)
    max_x, max_y = np.ceil(local_points.max(axis=0)).astype(int)
    width = max(1, max_x - min_x + 3)
    height = max(1, max_y - min_y + 3)
    local_mask = np.zeros((height, width), dtype=np.uint8)
    shifted = np.round(local_points - np.array([min_x - 1, min_y - 1])).astype(int)
    shifted[:, 0] = np.clip(shifted[:, 0], 0, width - 1)
    shifted[:, 1] = np.clip(shifted[:, 1], 0, height - 1)
    local_mask[shifted[:, 1], shifted[:, 0]] = 1
    kernel = np.ones((3, 3), dtype=np.uint8)
    local_mask = cv2.morphologyEx(local_mask, cv2.MORPH_CLOSE, kernel)

    local_origin = np.array([min_x - 1, min_y - 1], dtype=np.float32)
    local_center = center + local_origin @ inverse_rotation.T
    return local_mask, local_center, inverse_rotation


def cover_uncovered_regions(
    output: np.ndarray,
    component: np.ndarray,
    angle_degrees: float,
) -> np.ndarray:
    uncovered = (component > 0) & (output == 0)
    count, labels, stats, _ = cv2.connectedComponentsWithStats(
        uncovered.astype(np.uint8),
        connectivity=8,
    )
    min_area = max(4, int(component.sum() * 0.006))
    result = output.copy()
    for label in range(1, count):
        ...
        polygon = fixed_angle_rect_polygon(region, angle_degrees)
        if polygon is not None:
            fill_polygon(result, polygon)
    return result


def close_output_mask(mask: np.ndarray) -> np.ndarray:
    kernel = np.ones((3, 3), dtype=np.uint8)
    return cv2.morphologyEx(mask.astype(np.uint8), cv2.MORPH_CLOSE, kernel)


def decompose_local_rectangles(
    mask: np.ndarray,
    max_depth: int = 4,
    min_area: int = 12,
    overfill_threshold: float = 0.28,
) -> list[tuple[float, float, float, float]]:
    if int(mask.sum()) < min_area:
        return []

    x1, y1, x2, y2 = local_mask_bbox(mask)
    crop = mask[y1 : y2 + 1, x1 : x2 + 1]
    foreground = int(crop.sum())
    rectangle_area = int(crop.shape[0] * crop.shape[1])
    overfill_ratio = (rectangle_area - foreground) / max(foreground, 1)
    if max_depth <= 0 or overfill_ratio <= overfill_threshold:
        return [(float(x1), float(y1), float(x2), float(y2))]

    split = choose_local_split(crop, min_area=min_area)
    if split is None:
        return [(float(x1), float(y1), float(x2), float(y2))]
    ...
    if axis == "x":
        ...
        split_x = x1 + index
        first[y1 : y2 + 1, x1:split_x] = mask[y1 : y2 + 1, x1:split_x]
        second[y1 : y2 + 1, split_x : x2 + 1] = mask[y1 : y2 + 1, split_x : x2 + 1]
    else:
        first = np.zeros_like(mask)
        second = np.zeros_like(mask)
        split_y = y1 + index
        first[y1:split_y, x1 : x2 + 1] = mask[y1:split_y, x1 : x2 + 1]
        second[split_y : y2 + 1, x1 : x2 + 1] = mask[split_y : y2 + 1, x1 : x2 + 1]

    rectangles = decompose_local_rectangles(
        first,
        max_depth=max_depth - 1,
        min_area=min_area,
        overfill_threshold=overfill_threshold,
    )
    rectangles.extend(
...
def local_mask_bbox(mask: np.ndarray) -> tuple[int, int, int, int]:
    ys, xs = np.where(mask > 0)
    return int(xs.min()), int(ys.min()), int(xs.max()), int(ys.max())


def choose_local_split(mask: np.ndarray, min_area: int = 12) -> tuple[str, int] | None:
    best = None
    best_score = None
    projections = [
        ("x", mask.sum(axis=0)),
    ...
    for axis, projection in projections:
        length = projection.shape[0]
        if length < 6:
            continue
        for index in range(2, length - 2):
            before = int(projection[:index].sum())
            after = int(projection[index:].sum())
            if before < min_area or after < min_area:
                continue
            score = (int(projection[index]), abs(before - after))
...
def fixed_angle_rect_polygon(mask: np.ndarray, angle_degrees: float) -> np.ndarray | None:
    ...
    return local_bounds_to_polygon(
